import React, { useEffect, useState } from 'react';
import FilterInventoryStock from '../../components/admin/inventoryStock/FilterInventoryStock';
import InventoryStockTable from '../../components/admin/inventoryStock/InventoryStockTable';
import TabNavigation from '../../components/admin/TabNavigation';
import { useAppDispatch, useAppSelector } from '../../store/hooks';
import { fetchInventoryStocks } from '../../store/slices/inventoryStockSlice';
import { fetchStorageLocations } from '../../store/slices/storageLocationSlice';

const InventoryStockManagement: React.FC = () => {
  const dispatch = useAppDispatch();

  const { stocks, loading: isStockLoading } = useAppSelector((state) => state.inventoryStocks);
  const { storageLocations, loading: isLocLoading } = useAppSelector(
    (state) => state.storageLocations
  );

  const [tabIndex, setTabIndex] = useState(0);

  useEffect(() => {
    dispatch(fetchInventoryStocks());
    dispatch(fetchStorageLocations());
  }, [dispatch]);

  const tableHeads = [
    'Product name',
    'Location',
    'Quantity',
    'Batch No',
    'Expiry Date',
    'Serial Number',
  ];

  const getTabColor = (index: number) => {
    if (index === tabIndex) return '#ffffff';
    const lightness = Math.max(92 - index * 4, 60);
    return `hsl(215, 20%, ${lightness}%)`;
  };

  const isLoading = isStockLoading || isLocLoading;

  return (
    <div className="w-full pl-75 pr-10">
      <div>
        <h1 className="text-[22px] font-semibold mb-1.25">Inventory Stock Management</h1>
        <p className="text-[13px] text-wms-muted mb-6.25">
          View warehouse inventory, locations, and quantities
        </p>
      </div>

      <div className="bg-transparent flex flex-col overflow-x-auto">
        <TabNavigation
          tabs={['All stocks', 'Low stock alerts']}
          activeTabIndex={tabIndex}
          onTabChange={setTabIndex}
          getTabColor={getTabColor}
        />

        <div className="w-full bg-white rounded-r-2xl rounded-bl-2xl p-6.25 shadow-[0_4px_15px_rgba(0,0,0,0.03)] overflow-x-auto">
          <FilterInventoryStock />

          {isLoading ? (
            <div className="py-10 text-center">Loading data...</div>
          ) : (
            <InventoryStockTable heads={tableHeads} data={stocks} locations={storageLocations} />
          )}
        </div>
      </div>
    </div>
  );
};

export default InventoryStockManagement;
