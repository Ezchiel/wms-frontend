        ...
        {users.length > 0 ? (
          users.map((user) => (
            <tr key={user.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="py-4.5 px-3.75 border-b border-b-wms-border-color text-wms-text-main">
                <span
                  className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${user.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : user.status === 'LOCKED' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}`}
                >
                  {user.status}
                </span>
              </td>
